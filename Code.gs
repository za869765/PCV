 ...
 *  內容在格內展開（不再跨列），格內各自捲動
 * v4.8 變更：展開改聚焦模式——任一格展開時其他格縮成只剩標題列、
 ...
 * v5.2 變更：可點擊容器表頭改藍色漸層底（hover 加深）＋👆手勢引導動畫，
 *  與不可點區域明顯區別；未夾帶佔位表頭灰底無手勢；
 *  ⚙判讀設定回歸各自容器（新冠格=年齡對照表、流感格=門檻/代號/F代碼表，
 *  肺鏈免填不設；分域儲存互不覆寫，共用格移除總設定鈕）
 * v5.3 變更：流感表頭元件互換——「目前設定是＋切換」靠左（檔名旁）、
 *  「⚙ 判讀設定」與新冠一致靠最右
 * v5.4 變更：主頁三張 HIS 卡加高（min-height 150、padding 22、夾帶區 58px）；
 *  手動新增記憶候選依疫苗類別隔離（datalist 每格依 family 過濾：新冠>新冠、
